...
\label{eq:generic-dispersion}
\end{equation}

Por tanto, las bandas se tocan si y solo si
\begin{equation}
d_x(\vk)=d_y(\vk)=d_z(\vk)=0.
\label{eq:crossing-condition}
\end{equation}

\subsection{Por que 3D cambia el juego}

La condicion \eqref{eq:crossing-condition} son tres ecuaciones.
...
Sustituyendo en \eqref{eq:generic2band},
\begin{equation}
H(\vk_0+\vq)\approx \sum_{i,j} A_{ij}q_j \sigma_i.
\label{eq:weyl-general}
\end{equation}
...
\qquad \chi=\pm 1,
\end{equation}
entonces \eqref{eq:weyl-general} se reduce a
\begin{equation}
H_\chi(\vq)=\chi\, v \,(q_x\sigma_x + q_y\sigma_y + q_z\sigma_z).
\label{eq:weyl-isotropic}
\end{equation}

Sus autovalores son
\begin{equation}
E_\pm(\vq)=\pm v\sqrt{q_x^2+q_y^2+q_z^2}.
\label{eq:weyl-dispersion}
\end{equation}
...
El signo \(\chi=\pm 1\) en \eqref{eq:weyl-isotropic} se llama quiralidad o carga quiral del nodo. De manera intuitiva:
...
Los autovalores de \eqref{eq:dirac4x4} son
\begin{equation}
E_\pm(\vq)=\pm v\sqrt{q_x^2+q_y^2+q_z^2},
\end{equation}
