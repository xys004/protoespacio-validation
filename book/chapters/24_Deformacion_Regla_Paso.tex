...
Sustituyendo esto en \eqref{eq:UWdeformed} y reteniendo solo terminos lineales en \(\varepsilon\) y en \(\vk\), obtenemos
\begin{equation}
U_W(\varepsilon,\vk)
\simeq
\bbI
...
La ecuacion \eqref{eq:first-linear} adopta la forma
\begin{equation}
U_W(\varepsilon,\vk)
\simeq
...
La expresion \eqref{eq:HeffWeyl1} muestra tres clases de efectos.
...
\label{eq:DiracDeformed}
\end{equation}

Comparado con \eqref{eq:Dirac0}, vemos tres escenarios:
\begin{enumerate}[label=\arabic*.]
...
Lo que emerge de \eqref{eq:anisotropicWeyl} y \eqref{eq:DiracDeformed} es muy importante para la pregunta del protoespacio.
