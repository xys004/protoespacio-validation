...
\abs{\omega^\mu{}_\nu}\ll 1.
\end{equation}
Al imponer \eqref{c16:eq:lorcond} a primer orden, se obtiene
\begin{equation}
\omega_{\mu\nu} = -\omega_{\nu\mu}.
\label{c16:eq:antisym}
\end{equation}
...
\begin{displayquote}
\textbf{Que eleccion de $\Sigma^{\mu\nu}$ hace verdadera la condicion \eqref{c16:eq:covgamma}?}
\end{displayquote}
...
Ahora queremos ver que \eqref{c16:eq:sigmadef} realmente implementa \eqref{c16:eq:covgamma}.
...
Usando \eqref{c16:eq:sigmadef}, el conmutador buscado satisface la identidad
\begin{equation}
...
Sustituimos \eqref{c16:eq:term1} y \eqref{c16:eq:term2} en \eqref{c16:eq:longcomm}. Los terminos cubicos se cancelan exactamente y queda
\begin{align}
[\Sigma^{\rho\sigma},\gamma^\mu]
&=
\frac{i}{4}
...
Esta expresion es la misma que \eqref{c16:eq:master}, salvo un intercambio de nombres mudos y un signo global equivalente por orden de indices. La forma estandar que usaremos es
\begin{equation}
[\Sigma^{\rho\sigma},\gamma^\mu]
=
i(\eta^{\mu\rho}\gamma^\sigma-\eta^{\mu\sigma}\gamma^\rho).
...
Insertamos ahora esta identidad en \eqref{c16:eq:firstorder}:
\begin{align}
S^{-1}\gamma^\mu S
&=
\gamma^\mu
...
\gamma^\mu+\omega^\mu{}_\nu\gamma^\nu,
\end{equation}
que coincide exactamente con \eqref{c16:eq:vectorial}. Por tanto, la definicion \eqref{c16:eq:sigmadef} es correcta.
...
Con \eqref{c16:eq:Jmatrix} y \eqref{c16:eq:Kmatrix},
\begin{align}
[J_i,K_j]
&=
\frac{i}{4}
...
Si insertamos \eqref{c16:eq:Jmatrix} y \eqref{c16:eq:Kmatrix}, vemos que un sector quiral queda asociado a una de esas copias, y el otro sector a la otra. Esa es la forma algebraica precisa de decir que:
\begin{enumerate}[label=\arabic*.]
