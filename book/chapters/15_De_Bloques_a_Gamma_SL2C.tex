...
Para que \eqref{eq:hamiltonevo} describa una particula relativista libre con espectro
\begin{equation}
...
Con las definiciones \eqref{eq:alphabeta}, estas relaciones salen directamente de las reglas de Pauli:
\begin{equation}
...
Ahora podemos verificar \eqref{eq:cliff} de forma directa:
\begin{align}
...
Partimos de \eqref{eq:hamiltonevo}:
\begin{equation}
i\partial_t \Psi =
\left(
...
\right)\Psi=0.
\end{equation}
Usando \eqref{eq:defgamma}, esto queda
\begin{equation}
\left(
...
\end{equation}
la ecuacion \eqref{eq:almostcov} toma la forma
\begin{equation}
\left(
...
\begin{enumerate}[label=\arabic*.]
    ...
    \item el Hamiltoniano en bloques se traduce exactamente a la ecuacion covariante \eqref{eq:diraccov};
