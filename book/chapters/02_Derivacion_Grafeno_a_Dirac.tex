...
% verified-by: validators/graphene.py::test_phi_vanishes_at_K
% verified-by: validators/graphene.py::test_gradient_modulus_is_dirac
% verified-by: validators/graphene.py::test_cross_term_is_imaginary
% verified-by: validators/graphene.py::test_fermi_velocity
...
La expresion \eqref{c02:eq:tb-real} dice algo muy concreto: un electron sentado en \(A\) puede saltar a cualquiera de sus tres vecinos \(B\), y viceversa. Toda la estructura no trivial de grafeno aparece porque esos tres caminos interfieren entre si cuando pasamos al espacio de momentos.
...
\label{c02:eq:fourier}
\end{equation}
con \(N\) el numero de celdas unitarias.

Sustituyendo \eqref{c02:eq:fourier} en \eqref{c02:eq:tb-real} se obtiene
\begin{align}
H
&=
...
a_{\vk}^\dagger b_{\vk'}
+ \text{h.c.}
\label{c02:eq:before-orthogonality}
\end{align}
...
Entonces \eqref{c02:eq:hk-sum} se escribe como
\begin{equation}
H = \sum_{\vk}
\begin{pmatrix}
...
\begin{equation}
E_\pm(\vk)=\pm t |f(\vk)|.
\label{c02:eq:dispersion-general}
\end{equation}
...
Usando los vectores \eqref{c02:eq:deltas},
\begin{align}
f(\vk)
&=
\ee^{-\ii a k_y}
...
\label{c02:eq:graphene-dispersion}
\end{equation}
...
Sustituyendo en \eqref{c02:eq:linear-term-only},
\begin{align}
f(\vK+\vq)
&\approx
\ii \Bigg[
...
\label{c02:eq:fK-final}
\end{align}

\subsection{Hamiltoniano efectivo cerca de K}

Insertando \eqref{c02:eq:fK-final} en \eqref{c02:eq:Hk},
\begin{equation}
H_K(\vq)
\approx
\frac{3at}{2}
...
La ecuacion \eqref{c02:eq:dirac-dispersion} es la firma de fermiones relativistas efectivos sin masa. La velocidad de la luz \(c\) de la teoria relativista es reemplazada aqui por la velocidad de Fermi \(v_F\), y las matrices de Pauli actuan sobre el pseudospin de subred en lugar del espin real.
...
Usando \eqref{c02:eq:valley-compact},
\begin{equation}
