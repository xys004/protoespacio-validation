...
\label{eq:ssh-fourier}
\end{equation}
donde \(a\) es la longitud de la celda y \(N\) el número de celdas.

Sustituimos \eqref{eq:ssh-fourier} en \eqref{eq:ssh-real}. El término intracelda queda
\begin{equation}
...
Sustituyendo \eqref{eq:ssh-dxdy},
\begin{equation}
E_\pm(k)=
...
Sustituyendo en \eqref{eq:ssh-dxdy},
\begin{align}
d_x(k)
&\approx t_1-t_2,
\\
...
Entonces \eqref{eq:ssh-linear} se escribe como
\begin{equation}
H(q)\approx m\,\sigma_x - v q\,\sigma_y.
\label{eq:ssh-massive-dirac}
\end{equation}
...
Del Hamiltoniano \eqref{eq:ssh-massive-dirac}, el espectro es
\begin{equation}
E_\pm(q)=\pm \sqrt{m^2+v^2 q^2}.
\label{eq:ssh-effective-disp}
\end{equation}
