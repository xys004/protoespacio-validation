...
El termino \eqref{eq:bterm} no es un adorno. Tiene una funcion estructural:
\begin{enumerate}[label=\arabic*.]
...
Un nodo de Weyl aparece cuando una de estas energias se hace cero. De \eqref{eq:axisenergy} obtenemos la condicion
\begin{equation}
b^2=
...
El espectro conocido de \eqref{eq:lightH} es
\begin{equation}
E(\bm{q})=
\pm
\sqrt{
...
Al expandir \eqref{eq:lowspec} cerca de cada nodo se obtiene
\begin{equation}
E^2 \simeq
...
La matriz de velocidades de \eqref{eq:Weylpair} es
\begin{equation}
...
Alrededor de cada uno de esos sectores, el Hamiltoniano efectivo tiene la misma estructura que \eqref{eq:lightH}, pero con \(m\) reemplazado por \(M_n\). Por tanto el criterio de desdoblamiento en ese sector es
\begin{equation}
|b|>|M_n|.
\label{eq:criterionMn}
\end{equation}
